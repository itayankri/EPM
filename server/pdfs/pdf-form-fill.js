// TODO: This module should be rewritten. It was taken from an external github and some parts of it might not be useful.
...
function updateOptionButtonValue(handles,fieldDictionary,value) {
    ...
    if(isWidget || ! fieldDictionary.exists('Kids')) {
        ...
        let modifiedDict = startModifiedDictionary(handles,fieldDictionary,{'V':-1,'AS':-1});
        let appearanceName;
        if(value === null) {
            ...
            let apDictionary = handles.reader.queryDictionaryObject(fieldDictionary,'AP').toPDFDictionary();
            let nAppearances = handles.reader.queryDictionaryObject(apDictionary,'N').toPDFDictionary().toJSObject();
        ...
        let modifiedDict = startModifiedDictionary(handles,fieldDictionary,{'V':-1,'Kids':-1});
        let kidsArray = handles.reader.queryDictionaryObject(fieldDictionary,'Kids').toPDFArray();
        let appearanceName;
        if(value === null) {
            // false is easy, just write '/Off' as the value and as the appearance stream
            appearanceName = 'Off';
            ...
            // grab the non off value. that should be the yes one
            let widgetDictionary = handles.reader.queryArrayObject(kidsArray,value).toPDFDictionary();
            let apDictionary = handles.reader.queryDictionaryObject(widgetDictionary,'AP').toPDFDictionary();
            let nAppearances = handles.reader.queryDictionaryObject(apDictionary,'N').toPDFDictionary().toJSObject();
            appearanceName = _.find(Object.keys(nAppearances),function(item){return item !== 'Off'});
        }
	...
	modifiedDict.writeKey('Kids')
        ...
        let fieldsReferences = writeKidsAndEndObject(handles,modifiedDict,kidsArray);
        ...
        for(let i=0;i<fieldsReferences.length;++i) {
            ...
            let modifiedFieldDict = startModifiedDictionary(handles,sourceField,{'AS':-1});
            if(value === i) {
...
function getOriginalTextFieldAppearanceStreamCode(handles, fieldDictionary) {
    ...
    if(!appearanceParent.exists('AP'))
        return null;
    let appearance = handles.reader.queryDictionaryObject(appearanceParent,'AP').toPDFDictionary();
    if(!appearance.exists('N'))
        return null;

    let appearanceXObject = handles.reader.queryDictionaryObject(appearance,'N').toPDFStream();
...
function writeAppearanceXObjectForText(handles,formId,fieldsDictionary,text,inheritedProperties) {
    let rect = handles.reader.queryDictionaryObject(fieldsDictionary,'Rect').toPDFArray().toJSArray();
    ...
    let originalAppearanceContent = getOriginalTextFieldAppearanceStreamCode(handles,fieldsDictionary);
    let before = '';
    let after = '';
    ...
    if(textOptions) {
        ...
        let textDimensions = textOptions.font.calculateTextDimensions(text,textOptions.size);
    ...
    if(handles.acroformDict.exists('DR')) {
        handles.writer.getEvents().once('OnResourcesWrite',function(args){
            ...
            let dr = handles.reader.queryDictionaryObject(handles.acroformDict,'DR').toPDFDictionary().toJSObject();
...
function writeFieldWithAppearanceForText(handles,targetFieldDict,sourceFieldDictionary,appearanceInField,textToWrite,inheritedProperties) {
    ...
    if(appearanceInField) {
        ...
        let kidsArray = handles.reader.queryDictionaryObject(sourceFieldDictionary,'Kids').toPDFArray();
        let fieldsReferences = writeKidsAndEndObject(handles,targetFieldDict,kidsArray);
        ...
        let modifiedDict = startModifiedDictionary(handles,sourceField,{'AP':-1});
        modifiedDict
            .writeKey('AP');
...
function updateTextValue(handles,fieldDictionary,value,isRich,inheritedProperties) {
    ...
    let modifiedDict = startModifiedDictionary(handles,fieldDictionary,fieldsToRemove);
...
function writeFieldAndKids(handles,fieldDictionary,inheritedProperties,baseFieldName) {
    ...
    let modifiedFieldDict = startModifiedDictionary(handles,fieldDictionary,{'Kids':-1});
...
function writeFilledForm(handles,acroformDict) {
    let modifiedAcroFormDict = startModifiedDictionary(handles,acroformDict,{'Fields':-1});
...
function fillForm(writer,data, options) {
    ...
    let acroformDict = catalogDict.exists('AcroForm') ? reader.queryDictionaryObject(catalogDict,'AcroForm'):null;
    ...
    if(acroformInCatalog.getType() === hummus.ePDFObjectIndirectObjectReference) {
        ...
        let acroformObjectId = acroformInCatalog.toPDFIndirectObjectReference().getObjectID();
        ...
        let catalogObjectId = reader.getTrailer().queryObject('Root').toPDFIndirectObjectReference().getObjectID();
